...
void sTemplateNPC::ApplyGlyph(Player* player, uint8 slot, uint32 glyphID, uint8 spec)
{
	if (GlyphPropertiesEntry const* gp = sGlyphPropertiesStore.LookupEntry(glyphID))
	{
		if (uint32 oldGlyph = player->GetGlyph(slot, spec))
		{
...
void sTemplateNPC::LearnTemplateGlyphs(Player* player)
{
	...
	{
		if ((*itr)->playerClass == GetClassString(player).c_str() && (*itr)->playerSpec == sTalentsSpec)
			ApplyGlyph(player, (*itr)->slot, (*itr)->glyph, (*itr)->spec);
	}
	player->SendTalentsInfoData(false);
}
